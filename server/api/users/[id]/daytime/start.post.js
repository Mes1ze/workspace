...
export default defineEventHandler(async (event) => {
    try {
        ...
        if(ctx.data.employee_id != ctx.user.id && !(ctx.data.comment ?? "").trim()) {
            ...
            return helper.resFormat(400, "При открытии смены за другого сотрудника вы обязаны указать комментарий.")
        }
        
        const result = await daytimeHistoryService.startDaytime(ctx)
